...
# These two files are generated by evaluate_model.py in the training Colab,
# then downloaded and committed to this same repo (root folder). They let
# the app show real evaluation metrics without needing the dataset or
# Colab available at runtime.
METRICS_PATH = "evaluation_outputs/metrics_summary.json"
CONFUSION_MATRIX_PATH = "evaluation_outputs/confusion_matrix.png"
...
def load_metrics_summary(metrics_path):
    """Load pre-computed evaluation metrics generated by evaluate_model.py.
    Returns None if the file hasn't been added to the repo yet."""
    ...
    with st.expander("📊 Model Performance (evaluation metrics)", expanded=False):
        if metrics is None:
            st.info(
                ...
                "the training Colab, then copy `evaluation_outputs/` into "
                "this repo to show real mAP / precision / recall / "
                "confusion matrix here."
            )
            return

        st.caption(f"Evaluated on the **{metrics['split_evaluated']}** split")

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Precision", f"{metrics['precision']:.3f}")
        c2.metric("Recall", f"{metrics['recall']:.3f}")
        c3.metric("mAP50", f"{metrics['map50']:.3f}")
        c4.metric("mAP50-95", f"{metrics['map50_95']:.3f}")
...
def main():
    ...
    # Model performance is shown up top, before any upload, so it's visible
